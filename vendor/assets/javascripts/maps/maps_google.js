...
( function($) {
	$.extend($.ui.gmap.prototype, {
		pagination: function(prop) {
			var $el = $("<div id='pagination' class='navbar' style='width: 100%'>" +
										"<div class='navbar-inner' style='padding: 0;'>" +
											"<ul class='nav' style='width: 100%;'>" +
												"<li style='width: 33%;'>" +
													"<a href='#' style='font-size: 20pt;' class='back-btn left'>" +
														"&lsaquo;" +
													"</a>" +
												"</li>" +
												"<li style='width: 33%;' class='center'>" +
													"<a class='display' style='font-size: 14pt;'>" +
													"</a>" +
												"</li>" +
												"<li style='width: 33%;'>" +
													"<a href='#' class='fwd-btn right' style='font-size: 20pt;'>" +
														"&rsaquo;" +
													"</a>" +
												"</li>" +
											"</ul>" +
...
$(function()
{
	...
	if ($('#google-map-extend-pagination').length)
	{
		...
		// var myTitlePlace = 
		var markers = [
			{'position': '-6.893398,107.605188', 'title': 'coba' }
		];
	...
	{
		$('#google-map-streetview').gmap({ 
		 'zoom':16,
		 'center':new google.maps.LatLng($(".vevent .latitude .value-title").attr('title'), $(".vevent .longitude .value-title").attr('title')),
		 'callback': function() {
			var self = this;
			self.microformat('.vevent', function(result, item, index) {
				...
				self.addMarker( { 'position': latlng, 'title': result.summary, 'icon': '/assets/bts_icon_sap.png' }, function(map, marker) {
