        ...
        ms.find('optgroup, option').each(function(){
          if ($(this).is('optgroup')){
            ...
            var selectableLi = $('<li '+attributes+'><span>'+$(this).text()+'</span></li>'),
                selectedLi = $('<li '+attributes+'><span class="ui-icon ui-icon-arrowthick-2-n-s"></span><span>'+$(this).text()+'</span></li>');

            var value = $(this).val();
            selectableLi.addClass('ms-elem-selectable').attr('id', value+'-selectable');
            selectedLi.addClass('ui-state-default ms-elem-selection').attr('id', value+'-selection').hide();
